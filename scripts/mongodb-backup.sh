...
print(dbs.join(' '));
EOF
      DB_BACKUPS=$(mongosh -u ${DB_USER} -p ${DB_PASSWORD} --authenticationDatabase=admin --quiet \
        mongodb://${DB_HOST}:${DB_PORT}/admin ${SCRIPT})
    fi
    ...
    for DB in ${DB_BACKUPS}; do
      ...
      if [ "${BACKUP}" = "full" ]; then
        ...
        ${MONGODUMP} -h ${DB_HOST}:${DB_PORT} -u ${DB_USER} -p ${DB_PASSWORD} --authenticationDatabase=admin \
          -j 1 --gzip --out=${BACKUPDIR} --db=${DB}
...
EOF
        ${MONGODUMP} -h ${DB_HOST}:${DB_PORT} -u ${DB_USER} -p ${DB_PASSWORD} --authenticationDatabase=admin \
          -j 1 --gzip --out=${BACKUPDIR} --db=${DB} --collection=fs.files --queryFile=${QUERY}
...
print(res.join(','));
EOF
        IDS=$(mongosh -u ${DB_USER} -p ${DB_PASSWORD} --authenticationDatabase=admin --quiet \
          mongodb://${DB_HOST}:${DB_PORT}/${DB} ${SCRIPT})
        if [ -n "$IDS" ]; then
          cat << EOF > ${QUERY}
{"files_id":{"\$in":[${IDS}]}}
EOF
          ${MONGODUMP} -h ${DB_HOST}:${DB_PORT} -u ${DB_USER} -p ${DB_PASSWORD} --authenticationDatabase=admin \
            -j 1 --gzip --out=${BACKUPDIR} --db=${DB} --collection=fs.chunks --queryFile=${QUERY}
        fi
      fi
    done
  fi
