...
[ -f "${ENV}" ] && {
  ...
  if [ -n "${MYSQLDUMP}" -a -n "${DB_BACKUPS}" ]; then
    ...
    if [ "${DB_BACKUPS}" = "*" ]; then
      DB_BACKUPS=$(echo "SHOW DATABASES;" | \
        mysql -h ${DB_HOST} -P ${DB_PORT} -u ${DB_USER} -p${DB_PASSWORD} --silent)
    fi
    ...
    for DB in ${DB_BACKUPS}; do
      ...
      ${MYSQLDUMP} --single-transaction --routines --quick --set-gtid-purged=OFF \
        -h ${DB_HOST} -P ${DB_PORT} -u ${DB_USER} -p${DB_PASSWORD} ${DB} | \
        ${ZZ} a -si "${DB_BACKUP}"
    done
