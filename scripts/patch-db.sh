...
create_patch_db() {
  local DB=$1
  local HASDB=$(echo "SELECT COUNT(*) FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='${DB}';" | \
...
VER=$(echo "SELECT ver FROM ${PATCHDB}.patchinfo WHERE id IN (SELECT MAX(id) FROM ${PATCHDB}.patchinfo);" | \
  mysql -u root -p${MYSQL_ROOT_PASSWORD} --silent)
VER=${VER:-0}
NVER=${VER}
...
if [ "${NVER}" -gt "${VER}" ]; then
  ...
  echo "INSERT INTO ${PATCHDB}.patchinfo VALUES (NULL, ${NVER}, NULL);" | \
